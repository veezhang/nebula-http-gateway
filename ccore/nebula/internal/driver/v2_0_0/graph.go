...
import (
	...
	graph2_0_0 "github.com/vesoft-inc/nebula-http-gateway/ccore/nebula/internal/thrift/v2_0_0/graph"
...
type (
	defaultGraphClient struct {
		graph *graph2_0_0.GraphServiceClient
	}

	authResponse struct {
		*graph2_0_0.AuthResponse
	}

	executionResponse struct {
		*graph2_0_0.ExecutionResponse
	}
)
...
func newGraphClient(transport thrift.Transport, pf thrift.ProtocolFactory) types.GraphClientDriver {
	return &defaultGraphClient{
		graph: graph2_0_0.NewGraphServiceClientFactory(transport, pf),
	}
}

func (c *defaultGraphClient) Open() error {
	return c.graph.Transport.Open()
}
